...
def change_state(n):
    ...
    state = n
    rospy.logout("Changed to state [{}]".format(n))
    if( n==0 ):
...
# Change to wall follow if there is an obstacle in front of you
        if(state==0):
            if (region["center"] < 0.9 and region["center"] > 0.15):
                ...
                change_state(1)
        if(state==1):
            print(only_calculate_err_dist(current_pos , shortest_obst_pos) )
            if(time_sec > 10 and only_calculate_err_dist(current_pos , shortest_obst_pos) < 2):
