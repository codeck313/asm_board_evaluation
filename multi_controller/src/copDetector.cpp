...
// Include tf2 for transformation
 ...
 // Topics
 static const std::string IMAGE_TOPIC = "/robot5/camera/camera";
 static const std::string POINT_CLOUD2_TOPIC = "/robot5/camera/camera";
...
std::vector<cv::Point2f> extract_centroids(cv::Mat canny_output) {
  ...
  cv::circle( drawing, centroids[i], 4, cv::Scalar(160,15,0), -1, 8, 0 );
  ROS_INFO_STREAM("Centre of the box is at"<< i<< " --> "  << centroids[i].x << " " << centroids[i].y<<"\n");
  }
