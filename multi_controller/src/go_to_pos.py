...
threshold_yaw = math.pi   # error of +-4degrees is allowed
threshold_distance = 0.5       # error of 40cm is allowed
...
# The below given step is called normalization. This makes the bot orient towards the goal in the least posible rotation
    ...
    if ( dist_offset > threshold_distance):
        msg_to_publish.linear.x = 0.1 #0.6
        msg_to_publish.angular.z = 0
        pub.publish(msg_to_publish)
    else:
        rospy.logout("error in yaw is : {}".format(dist_offset))
        change_state(2)
    
    if (math.fabs(angle_offset) > threshold_yaw):
        rospy.logout("error in yaw is : {}".format(angle_offset))
        change_state(0)
...
def correct_heading():
    ...
    if (math.fabs(angle_offset) > threshold_yaw):
        if angle_offset > 0 :
            msg_to_publish.angular.z = 0.1 #0.6
            msg_to_publish.linear.x = 0
            pub.publish(msg_to_publish)
        else : 
            msg_to_publish.angular.z =  -0.1 #-0.6
...
def main():
    ...
    pub = rospy.Publisher("/user/cmd_vel" , Twist , queue_size=10)
    sub = rospy.Subscriber("/best_odom" , Odometry , callback = calc_pos_rpy)
