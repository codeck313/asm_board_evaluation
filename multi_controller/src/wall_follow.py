...
def find_wall():
    global message
    message.linear.x = 0.1#0.4
    message.angular.z = -0.2 #-0.6
    pub.publish(message)

def go_left():
    global message
    message.linear.x = 0
    message.angular.z = 0.2 #0.3
    pub.publish(message)

def follow_wall():
    global message
    message.linear.x = 0.1#0.3
    message.angular.z = 0
    pub.publish(message)
...
def callback(msg):
    ...
    region ={
            "center" : min(min(msg.ranges[0:170]), min(msg.ranges[1776:1946]) , 2 ),
        "left" :  min(min(msg.ranges[171:510]) , 2 ),
        "eleft" : min(min(msg.ranges[511:850]) , 2 ),
        "right" :   min(min(msg.ranges[1436:1777]) , 2 ),
        "eright" : min(min(msg.ranges[1096:1437]) , 2 )
    }
    print(region)
...
def main():
    ...
    pub = rospy.Publisher("/user/cmd_vel" , Twist , queue_size=1)
